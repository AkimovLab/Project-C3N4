...
power = 8
nfiles = fstep-istep
path_to_npz_files = '../../3_nacs/10x10/res-mixed-basis-raw-ordered-identity'

params["path_to_npz_files"] = path_to_npz_files
active_space = list(range(0,72))
...
for i in range(istep,fstep):
    ...
    # Setting up the SAC basis
    hvib_im[:,0] *= np.sqrt(2)
    hvib_im[0,:] *= np.sqrt(2)
    # Multiplying all the NACs with sqrt(2)^n
    hvib_im = hvib_im*np.power(np.sqrt(2),power)
...
for icond in [igeo]:
    ...
    params_nbra = { "nsteps": user_nsteps, "dt": 1.0*units.fs2au, 
                    ...
                    "mem_output_level":-1,
                    "txt_output_level":3,
                    "properties_to_save": ['timestep', 'time', 'SH_pop'],
    ...
    prefix = F"{power}_namd_res_{icond}"
